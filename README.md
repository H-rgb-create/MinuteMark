...
1. 在“智能功能设置”中填写云端大模型的 API 地址和 API Key。OpenAI Chat Completions 接口默认使用 `gpt-4.1-mini`；如兼容服务需要其他模型，可在 API 地址末尾追加 `?model=模型名`。Anthropic Messages 兼容接口请追加 `?protocol=anthropic`，默认使用 `deepseek-v4-flash[1M]`。API Key 使用系统安全存储加密保存，不会写入会议数据。
